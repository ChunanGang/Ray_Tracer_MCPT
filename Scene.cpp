...
const int y_size = (y_max - y_min) / section;
const int x_size = (x_max - x_min) / section;
const int z_size = (z_max - z_min) / section;
...
void initScene(Sphere* cpu_spheres) {
	...
	cpu_spheres[1].position = Vector3Df(-0.25f, 0.16f, -0.1f);
	...
	cpu_spheres[5].position = Vector3Df(-0.0f, 1.8f, -0.0f);
